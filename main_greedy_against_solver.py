...
if __name__ == '__main__':
    ...
    SS = {}
    # SS = [5, 10, 20, 30, 40, 70, 90, 130, 160, 190]  # total number of service request
    SS[InfP[0]] = [5, 10, 20, 40, 50, 60, 80]  # total number of service request
    SS[InfP[1]] = [10, 20, 40, 60, 90, 120, 150]  # total number of service request
    SS[InfP[2]] = [20, 40, 70, 100, 130, 160, 200]  # total number of service request
    ...
    for r_type in resource_types:
        if r_type == 'IoT':
            ...
            resource_profile_small[r_type] = 10000 * x
            resource_profile_large[r_type] = 50000 * x
        elif r_type == 'Firehose':
            # maximum number of TBs that can be streamed into Firehose resources
            resource_profile_small[r_type] = 200 * x
            resource_profile_large[r_type] = 1000 * x
        # elif r_type == 'Analytics':
        #     # number of Data Analytics processing units available by each Provider in each region
        #     resource_profile_small[r_type] = 100 * x
        #     resource_profile_large[r_type] = 500 * x
        # elif r_type == 'S3':
        #     # maximum number of TBs can be stored by each Provider in each region
        #     resource_profile_small[r_type] = 500 * x
        #     resource_profile_large[r_type] = 2500 * x
        elif r_type == 'EMR':
            # number of vCPUs maintained by a single providers in a single location
            resource_profile_small[r_type] = 1000 * x
            resource_profile_large[r_type] = 5000 * x
        # elif r_type == 'Quick':
        #     # number of Qyicksight instances can be supported by a single providers in a single location
        #     resource_profile_small[r_type] = 10**6  * x # THIS IN FACT MEANS NO LIMITATION
        #     resource_profile_large[r_type] = 10**6 * x
